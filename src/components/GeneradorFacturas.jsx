...
const GeneradorFacturas = () => {
  ...
  const generarFacturas = async () => {
    try {
      const response = await axios.post('http://localhost:3000/api/generar-facturas');
  ...
  const updateInvoiceLinkState = async (invoiceLinkId, newState) => {
    try {
      const response = await axios.put(`http://localhost:3000/api/invoiceLinks/${invoiceLinkId}/state`, {
        state: newState,
      });
              ...
              filterAndSortFacturas(facturas).map(invoiceLink => (
                <tr key={invoiceLink._id}>
                  <td className="border p-2">
                    <a href={`http://localhost:3000/facturas/${invoiceLink.fileName}`} target="_blank" rel="noopener noreferrer">
