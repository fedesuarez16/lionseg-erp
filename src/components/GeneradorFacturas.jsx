...
const GeneradorFacturas = () => {
  ...
  const generarFacturas = async () => {
    try {
      const response = await axios.post('https://localhost:3000/api/generar-facturas');
  ...
  const fetchFacturas = async () => {
    try {
      const response = await axios.get('https://localhost:3000/api/clientes');
      if (response.status === 200) {
  ...
  const updateInvoiceLinkState = async (invoiceLinkId, newState) => {
    try {
      const response = await axios.put(`https://localhost:3000/api/invoiceLinks/${invoiceLinkId}/state`, {
        state: newState,
      });
              ...
              filterAndSortFacturas(facturas).map(invoiceLink => (
                <tr key={invoiceLink._id}>
                  <td className="border p-2">
                    <a href={`https://localhost:3000/facturas/${invoiceLink.fileName}`} target="_blank" rel="noopener noreferrer">
