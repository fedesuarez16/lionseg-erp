...
const ClientProfile = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3000/api/clientes/${clientId}`)
      .then((response) => {
  ...
  const handleFormSubmit = () => {
    axios.put(`http://localhost:3000/api/clientes/${clientId}`, formData)
      .then((response) => {
        setClient(response.data);
        setIsEditing(false);
      })
  ...
  const handleDeleteClient = () => {
    axios.delete(`http://localhost:3000/api/clientes/${clientId}`)
      .then(() => {
        navigate('/clients');
      })
      .catch((error) => {
