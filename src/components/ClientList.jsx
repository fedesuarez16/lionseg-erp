...
const ClientList = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/clientes?search=${searchQuery}`);
  ...
  const handleStateChange = async (clientId, newState) => {
    try {
      const response = await axios.put(`http://localhost:3000/api/clientes/${clientId}`, { state: newState });
