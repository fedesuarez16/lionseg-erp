...
const ClientList = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await axios.get(`https://https://lionseg-df2520243ed6.herokuapp.com/api/clientes?search=${searchQuery}`);
  ...
  const handleStateChange = async (clientId, newState) => {
    try {
      const response = await axios.put(`https://https://lionseg-df2520243ed6.herokuapp.com/api/clientes/${clientId}`, { state: newState });
